...
def data_rw(uart_inst,database_inst):
    try:
        # 重复执行数据读取写入'
        ...
        while True:
            ...
            for inst in uart_inst:
                ...
                uart_data[meter_addr] = data_list

            for key,value in uart_data.items():
                
                print("database is value:",value)
                
                database_inst['cmd'].executemany(query=SQL,args=uart_data[meter_addr])
...
def main():
    ...
    data_rw(uart_inst,database_inst)
